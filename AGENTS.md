...
TruckFarm 是基于旧版 Servlet + MiniUI 毕设项目持续迭代升级的农场经营数字沙盘。后续写代码、改文档、设计接口、拆任务和做简历化包装时必须遵守以下规则。

---

## 一、项目定位

TruckFarm 面向中小型农场、合作社或农产品经营模拟场景，提供农场经营过程中的种植、库存、采购、销售、人员、数据看板和数字沙盘模拟能力。

项目定位必须保持清晰：

- 主场景：农场经营数字沙盘，不是通用后台模板，也不是纯游戏。
- 核心目标：作物资料可维护、种植计划可追踪、采购库存可管理、销售收益可统计、时间环境可推进、操作结果可模拟。
- 简历目标：体现基于旧项目边改边学、逐步替换技术栈、持续重构为现代前后端分离工程的能力。
- AI 定位：辅助分析环境影响、生成修正因子和解释，不能作为核心业务依赖，不能直接决定最终业务数据。
- 不做方向：不做电商平台、不做农业物联网硬件平台、不做泛 ERP、不做纯后台管理脚手架、不做复杂 3D 游戏。

任何新增功能都必须服务于农场经营闭环和数字沙盘模拟，不允许随意扩展成宽泛的“管理系统”。
### 数字沙盘模拟原则

- 管理系统是底座，模拟引擎是核心亮点，经营决策是主线。
- 用户操作不直接产生固定结果，必须结合当前环境、业务规则、随机种子和可选 AI 修正。
- 本地规则必须先给出基础区间，AI 只能返回结构化修正因子、风险等级和原因解释。
- AI 输出必须经过后端校验、范围裁剪和 fallback，禁止直接修改库存、资金、订单状态等核心数据。
- 每次模拟结果必须保存输入上下文、基础规则、AI 修正、最终结果和 `random_seed`，保证可追踪、可复现。
- AI 调用失败不能阻塞采购、播种、养护、收获、销售等核心经营流程。
...
3. 旧版 `src/`、`pom.xml`、`table.sql` 在正式迁移前保持原样，不随意删除；永久留痕副本可放在 `old/`，未来正式迁移归档可整体移动到 `legacy/`，两者都必须保留来源说明。
...
│   │       ├── crop/                     # 作物档案、作物分类、生长规则
│   │       ├── field/                    # 地块档案、环境状态
│   │       ├── planting/                 # 种植计划、农事任务、收获记录、生长状态
│   │       ├── supplier/                 # 供应商管理、信誉参数
│   │       ├── procurement/              # 采购单、采购明细、采购模拟结果、入库
│   │       ├── inventory/                # 库存、出入库流水、库存质量、库存预警
│   │       ├── customer/                 # 客户档案、客户关系参数
│   │       ├── sales/                    # 销售订单、出库、成交结果
│   │       ├── finance/                  # 经营账户、资金流水、收支统计
│   │       ├── simulation/               # 时间推进、规则计算、随机采样、结果记录
│   │       ├── weather/                  # 天气生成、天气记录
│   │       ├── market/                   # 市场价格、行情波动
│   │       ├── event/                    # 风险事件、事件处理
│   │       ├── report/                   # 经营复盘、决策影响分析
│   │       ├── dashboard/                # 首页沙盘、趋势图、待办和预警信息
│   │       ├── ai/                       # 经营分析、结构化修正因子、数据总结
│   │       └── system/                   # 字典、配置、操作日志
│   │
│   └── src/main/resources/
...
- 兼容性验证结论必须直接记录到 `docs/tasks.md` 的 `P1.x` 小目标中。
---
...
- 首页沙盘必须围绕经营模拟闭环：当前日期、天气、地块状态、作物生长、事件、库存、资金和收益。
...
- AI 模拟输出必须是结构化 JSON，并保存调用上下文、输出、状态和错误信息。
...
public interface AiClient {
  ...
  SimulationModifier suggestSimulationModifier(SimulationContext context);
}
```
...
| 首页沙盘 | `truckfarm:dashboard:summary` | 首页沙盘缓存 |
...
- 关键页面：登录页、首页沙盘、地块沙盘、事件时间线、模拟动作结果、作物管理、地块管理、种植计划、采购管理、库存管理、销售订单、用户权限。

### 前端视觉方向

- 视觉风格应体现农场经营与数据管理结合，避免默认后台模板感过强。
- 色彩建议以自然绿色、土地色、中性灰为主，谨慎使用大面积高饱和色。
- 首页沙盘要有业务叙事：今天是什么天气、地块发生了什么、作物长到哪里、库存缺什么、销售赚多少、接下来做什么。
- 移动端至少保证登录、看板和核心列表可用。
...
- 文档应集中维护在现有 `README.md`、`AGENTS.md` 和 `docs/` 主文件中，避免新增过多平行文档。
...
- `docs/tasks.md` 是开发计划、阶段状态和留痕主记录，不额外拆平行开发日志文档。
...
2. 先完成核心模拟闭环：初始化农场 -> 采购种子 -> 播种 -> 推进日期 -> 触发事件 -> 收获入库 -> 销售 -> 经营复盘。
