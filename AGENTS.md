...
TruckFarm 是从旧版 Servlet + MiniUI 毕设项目推倒重制的智慧农场经营管理平台。后续写代码、改文档、设计接口、拆任务和做简历化包装时必须遵守以下规则。
...
- 简历目标：体现从传统 Java Web 项目升级为现代前后端分离工程的重构能力。
...
1. 旧版代码只作为业务参考和迁移依据，禁止在旧 Servlet 架构上继续堆功能。
2. 新版代码使用前后端分离结构，后端放 `backend/`，前端放 `frontend/`，文档放 `docs/`。
3. 旧版 `src/`、`pom.xml`、`table.sql` 在正式迁移前保持原样，不随意删除。
4. 如需要保留旧版代码，优先整体移动到 `legacy/`，并在 README 中说明来源。
5. 重制过程先补文档，再写代码；所有表结构、接口、页面和任务变更必须同步到 `docs/`。
...
**后端技术栈**：JDK 21 / Spring Boot 3.x / Spring Security / JWT / MyBatis Plus / MySQL 8 / Redis 7 / Spring Scheduler / MapStruct / SpringDoc OpenAPI 或 Knife4j / MinIO 或本地文件存储 / Maven

**前端技术栈**：Vue 3 / TypeScript / Vite / Element Plus / Pinia / Vue Router / Axios / ECharts

**工程化能力**：Docker Compose / Flyway 或 Liquibase / JUnit 5 / Mockito / AssertJ / GitHub Actions
...
- 金额使用 `DECIMAL`，禁止使用 `Double` / `Float` 表示金额。
...
| 禁止项 | 原因 |
| --- | --- |
| 在旧 Servlet 架构上继续堆新功能 | 重制目标是现代化架构，不是修补旧项目 |
...
项目当前处于旧项目评估和重制方案准备阶段。后续执行时必须遵守：

1. 先完成项目文档，再开始脚手架和业务代码。
