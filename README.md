...
这次升级不是完全另起炉灶做一个新项目，而是在旧项目基础上边重构、边检查旧代码不规范之处、边学习新技术替换旧技术：用 Spring Boot、MyBatis Plus、Spring Security、Vue 3 等现代技术逐步替代 Servlet、JDBC、Session、MiniUI 等旧实现，同时保留并演进旧项目中的业务概念、字段、菜单和页面流程。
...
升级过程中会保留并迁移旧项目中的“农作物、用户、部门、菜单权限”等业务概念；旧 Servlet、MiniUI、JDBC 实现会作为学习对照，不原样作为最终运行架构，而是逐步重构为 Spring Boot、MyBatis Plus 和 Vue 3 对应实现。
...
- 查询逻辑迁移：旧 DAO 中有价值的查询条件、分页、菜单树逻辑会改写为 MyBatis Plus / XML。
