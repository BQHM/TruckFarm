...
- 主键统一为 `id BIGINT`。
- 金额使用 `DECIMAL(12,2)`。
- 数量、面积使用 `DECIMAL(12,2)` 或 `DECIMAL(14,3)`。
- 时间使用 `DATETIME`，纯日期使用 `DATE`。
- 核心业务表使用逻辑删除字段 `deleted`。
...
| reference_price | DECIMAL(12,2) | 参考价格 |
...
| planned_area | DECIMAL(12,2) | 计划种植面积 |
...
### 6.2 tf_harvest_record

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | BIGINT PK | 收获记录 ID |
| plan_id | BIGINT | 种植计划 ID |
| crop_id | BIGINT | 作物 ID |
| field_id | BIGINT | 地块 ID |
| actual_harvest_date | DATE | 实际收获日期 |
| yield_quantity | DECIMAL(14,3) | 产量 |
| loss_quantity | DECIMAL(14,3) | 损耗 |
| unit | VARCHAR(16) | 单位 |
| remark | VARCHAR(255) | 备注 |
| created_at | DATETIME | 创建时间 |
| created_by | BIGINT | 创建人 |
...
### 7.3 tf_procurement_order_item

字段：`id`、`order_id`、`crop_id`、`quantity`、`unit`、`unit_price`、`amount`、`created_at`、`deleted`。

## 8. 库存表

### 8.1 tf_inventory_stock

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id | BIGINT PK | 库存 ID |
| crop_id | BIGINT | 作物 ID，唯一 |
| quantity | DECIMAL(14,3) | 当前库存 |
| unit | VARCHAR(16) | 单位 |
| alert_threshold | DECIMAL(14,3) | 预警阈值 |
| version | INT | 乐观锁版本 |
| created_at | DATETIME | 创建时间 |
| updated_at | DATETIME | 更新时间 |
| deleted | TINYINT | 逻辑删除 |

### 8.2 tf_inventory_flow
...
| quantity_change | DECIMAL(14,3) | 变动数量 |
| quantity_before | DECIMAL(14,3) | 变动前数量 |
| quantity_after | DECIMAL(14,3) | 变动后数量 |
| unit | VARCHAR(16) | 单位 |
...
### 9.3 tf_sales_order_item

字段：`id`、`order_id`、`crop_id`、`quantity`、`unit`、`unit_price`、`amount`、`created_at`、`deleted`。

## 10. 扩展表

### 10.1 tf_file_resource
