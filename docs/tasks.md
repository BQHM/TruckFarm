...
描述：复制旧版项目到 `old/`，作为可压缩归档的永久留痕包，并提供独立 Docker 一键部署方式。
...
验收标准：
- [x] `old/` 包含旧版 `src/`、`pom.xml`、`table.sql`、旧截图和旧版说明。
- [x] `old/` 包含 `Dockerfile`、`docker-compose.yml` 和 `legacy-init.sql`，可独立用于旧版部署。
- [x] `old/README.md` 说明启动、停止、重置和归档用途。
- [x] `old/` 不包含 `target/`、`.class`、`.war` 等构建产物。

验证方式：
- [x] `cd old; docker compose config`。
- [x] `cd old; mvn -q -DskipTests package` 本地构建通过，构建产物验证后已删除。
- [ ] Docker daemon 当前未启动，待 Docker Desktop 启动后执行 `cd old; docker compose up -d --build` 做运行验证。
...
完成记录：已创建 `old/` 旧版留痕包；Compose 配置校验通过，本地 Maven 构建通过，Docker 运行验证待 Docker daemon 可用后补充。
