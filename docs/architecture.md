...
TruckFarm 重制版采用前后端分离架构，目标是让项目具备真实简历项目所需的工程完整度：

- 后端使用 Spring Boot 3 构建模块化单体应用。
- 前端使用 Vue 3 + TypeScript 构建管理后台。
- 数据库使用 MySQL 8，缓存使用 Redis 7。
- 接口统一使用 RESTful + JSON。
...
```text
frontend dev server -> backend Spring Boot -> MySQL / Redis
```

演示环境：

```text
Nginx static frontend -> backend container -> MySQL container / Redis container
```
...
Mapper 使用 MyBatis Plus `BaseMapper<XxxEntity>`。简单 CRUD 使用内置方法，复杂 SQL 写在 XML 中。排序字段必须使用白名单，查询必须考虑 `deleted = 0`。
...
| 旧项目代码干扰新版 | 旧代码作为 legacy 保留，新代码放 `backend/` 和 `frontend/` |
