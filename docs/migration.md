...
根据 2026-05-14 的留痕要求，当前已新增 `old/` 目录作为旧版项目的独立复制包，用于压缩归档和永久保存。它与未来可能的 `legacy/` 迁移目录职责不同：

- `old/`：旧版项目的静态留痕副本，包含旧源码、旧 SQL 线索、旧截图、旧启动说明和可一键 Docker 部署配置。
- `legacy/`：未来新版脚手架稳定后，如需在仓库内归档旧工作目录，可作为迁移参考目录。

`old/` 不承载新版开发，不在其中继续堆功能；如需验证旧版，只在 `old/` 内执行 `docker compose up -d --build`。
